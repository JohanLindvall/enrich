...
func TestEnrich_ResourceId_ResourceGroup(t *testing.T) {
	enriched := Enrich(microsoftInsightsLog)
...
func TestEnrich_PlainText(t *testing.T) {
	...
	enriched := Enrich(plainTextLog)
	assert.Equal(t, "info", enriched.Severity)
	assert.Equal(t, "0001-01-01 00:00:00 +0000 UTC", enriched.Time.String())
}

func TestEnrich_LaunchDarkly(t *testing.T) {
	...
	enriched1 := Enrich(launchDarkly1)
	enriched2 := Enrich(launchDarkly2)
...
func TestEnrich_Vector(t *testing.T) {
	enriched := Enrich(`2024-10-08T06:42:37.720009Z ERROR sink{component_kind="sink" component_id=loki_sink component_type=loki}:request{request_id=586}: vector_common::internal_event::component_events_dropped: Events dropped intentional=false count=274 reason="Service call failed. No retries or retries exhausted." internal_log_rate_limit=true"`)
	assert.Equal(t, "error", enriched.Severity)
	assert.Equal(t, "2024-10-08 06:42:37.720009 +0000 UTC", enriched.Time.String())
}

func TestEnrich_TurborepoRemoteCache(t *testing.T) {
	enriched := Enrich(`{"severity":"INFO","level":30,"time":1728368183944,"pid":7,"hostname":"remote-cache-0","reqId":"Jqbyb5psTOSlYycP47IzBw-138328","res":{"statusCode":200},"responseTime":0.2516399919986725,"message":"request completed"}`)
	assert.Equal(t, "info", enriched.Severity)
	assert.Equal(t, "2024-10-08 06:16:23.944 +0000 UTC", enriched.Time.String())
}

func TestEnrich_NgAlert(t *testing.T) {
	enriched := Enrich(`logger=ngalert.sender.router rule_uid=XXYfpoNVk org_id=1 t=2024-10-08T14:42:46.051254789Z level=info msg="Sending alerts to local notifier" count=3"`)
	assert.Equal(t, "info", enriched.Severity)
	assert.Equal(t, "2024-10-08 14:42:46.051254789 +0000 UTC", enriched.Time.String())
}

func TestEnrich_Promtail(t *testing.T) {
	enriched := Enrich(`ts=2024-10-08T14:43:38.589263678Z caller=log.go:168 level=info msg="Waiting for /var/log/host/gh-diag/cbuild/pages/00000000-0000-0000-0000-000000000017_00000000-0000-0000-0000-000000000018_1.log to appear..."`)
	assert.Equal(t, "info", enriched.Severity)
	assert.Equal(t, "2024-10-08 14:43:38.589263678 +0000 UTC", enriched.Time.String())
}

func TestEnrich_NginxErr(t *testing.T) {
	enriched := Enrich(`2024/10/08 14:52:47 [error] 1394450#1394450: unexpected DNS response for oauth2-proxy.ingress.svc.cluster.local`)
	assert.Equal(t, "error", enriched.Severity)
	assert.Equal(t, "2024-10-08 14:52:47 +0000 UTC", enriched.Time.String())
}

func TestEnrich_EnvoyInfo(t *testing.T) {
	enriched := Enrich(`[2024-10-08 14:50:33.337][1][info][upstream] [source/common/upstream/cds_api_helper.cc:71] cds: added/updated 0 cluster(s), skipped 60 unmodified cluster(s)`)
	assert.Equal(t, "info", enriched.Severity)
	assert.Equal(t, "2024-10-08 14:50:33.337 +0000 UTC", enriched.Time.String())
}

func TestEnrich_CloseChannels(t *testing.T) {
	enriched := Enrich(`2024-10-08 14:55:10.587 [info] <0.7545.176> Closing all channels from connection '172.19.4.4:38958 -> 172.19.5.30:5672' because it has been closed`)
	assert.Equal(t, "info", enriched.Severity)
	assert.Equal(t, "2024-10-08 14:55:10.587 +0000 UTC", enriched.Time.String())
}

func TestEnrich_SkippingService(t *testing.T) {
	enriched := Enrich(`I1008 15:00:59.214583       1 utils.go:135] "Skipping service due to cluster IP" service="client-domain-reactions/domain-service-live" clusterIP="`)
	assert.Equal(t, "info", enriched.Severity)
	assert.Equal(t, "2024-10-08 15:00:59.214583 +0000 UTC"[4:], enriched.Time.String()[4:]) // Skipping year due to braindead klog format
}

func TestEnrich_Cloudflared(t *testing.T) {
	enriched := Enrich(`2024-10-08T06:31:22Z WRN Your version 2024.4.1 is outdated. We recommend upgrading it to 2024.9.1`)
	assert.Equal(t, "warn", enriched.Severity)
	assert.Equal(t, "2024-10-08 06:31:22 +0000 UTC", enriched.Time.String())
}

func TestEnrich_NginxIngress(t *testing.T) {
	enriched := Enrich(`203.0.113.11 - - [08/Oct/2024:06:42:27 -0700] "POST /api/completions HTTP/1.1" 200 658 "-" "RestSharp/110.2.0.0" 650 0.023 [base-search-service-search-service-5000] [] 10.244.1.16:5000 658 0.023 200 134828790ba4a09d748ed1526be8de50`)
	assert.Equal(t, "info", enriched.Severity)
	assert.Equal(t, "2024-10-08 13:42:27 +0000 UTC", enriched.Time.String())
}

func TestEnrich_DashboardMetricsScraper(t *testing.T) {
	enriched := Enrich(`10.244.2.51 - - [08/Oct/2024:06:43:18 +0000] "GET /healthz HTTP/1.1" 404 13 "" "dashboard/v2.7.0"`)
	assert.Equal(t, "warn", enriched.Severity)
	assert.Equal(t, "2024-10-08 06:43:18 +0000 UTC", enriched.Time.String())
}

func TestEnrich_KubernetesMetricsScraper(t *testing.T) {
	enriched := Enrich(`172.19.7.15 - - [08/Oct/2024:06:44:25 +0000] "GET /healthz HTTP/1.1" 200 13 "" "dashboard/v0.0.0 (linux/arm64) kubernetes/$Format"`)
	assert.Equal(t, "info", enriched.Severity)
	assert.Equal(t, "2024-10-08 06:44:25 +0000 UTC", enriched.Time.String())
}

func TestEnrich_Registry(t *testing.T) {
	enriched := Enrich(`172.19.6.253 - - [08/Oct/2024:06:43:06 +0000] "HEAD /v2/gha-script-base-client/blobs/sha256:42a07e277b07d2f6211f0666218223c018d1cfb9731d05c4d7549cf96f8857c9 HTTP/1.1" 200 0 "" "buildkit/v0.16"`)
	assert.Equal(t, "info", enriched.Severity)
	assert.Equal(t, "2024-10-08 06:43:06 +0000 UTC", enriched.Time.String())
}

func TestEnrich_MetricsScraper(t *testing.T) {
	enriched := Enrich(`10.244.5.1 - - [08/Oct/2024:06:55:21 +0000] "GET / HTTP/1.1" 200 6 "" "kube-probe/1.27"`)
	assert.Equal(t, "info", enriched.Severity)
	assert.Equal(t, "2024-10-08 06:55:21 +0000 UTC", enriched.Time.String())
}

func TestEnrich_Kubernetes_Event(t *testing.T) {
	enriched := Enrich(`name=domain-service-684dd55c5c-hmjc5 kind=Pod objectAPIversion=v1 objectRV=184488308 eventRV=188743881 sourcecomponent=kubelet sourcehost=aks-system-00000000-vmss000000 reason=Unhealthy type=Warning count=64853 msg="(combined from similar events): Liveness probe errored: rpc error: code = Unknown desc = failed to exec in container: failed to start exec \"428178d2be79d3602ca62ca6cca774a337aeeb933210abe15b9d80f148a1379f\": OCI runtime exec failed: exec failed: unable to start container process: exec: \"/liveness/liveness_check\": stat /liveness/liveness_check: no such file or directory: unknown""`)
	assert.Equal(t, "warn", enriched.Severity)
	assert.Equal(t, "0001-01-01 00:00:00 +0000 UTC", enriched.Time.String())
}

func TestEnrich_MsLog(t *testing.T) {
	enriched := Enrich(`[08/10/2024 17:00:08 INF Microsoft.Hosting.Lifetime 17  ] Application is shutting down..."`)
	assert.Equal(t, "info", enriched.Severity)
	assert.Equal(t, "2024-10-08 17:00:08 +0000 UTC", enriched.Time.String())
}

func TestEnrich_NginxUnprivileged(t *testing.T) {
	enriched := Enrich(`172.19.8.33 - - [08/Oct/2024:06:45:43 +0000]  204 "POST /loki/api/v1/push HTTP/1.1" 0 "-" "canary-push/k218-659f542" "-"`)
	assert.Equal(t, "info", enriched.Severity)
	assert.Equal(t, "2024-10-08 06:45:43 +0000 UTC", enriched.Time.String())
}

func TestEnrich_MS(t *testing.T) {
	enriched := Enrich(`[08/10/2024 06:00:07 ERR Acme.Entity.CasinoAvailability.Service.BlobContainerService 14  ] New data retrieved, uploading file entity-casinoavailability/v1/release/casino-availability.json.br"`)
	assert.Equal(t, "error", enriched.Severity)
	assert.Equal(t, "2024-10-08 06:00:07 +0000 UTC", enriched.Time.String())
}

func TestEnrich_ExternalDns(t *testing.T) {
	enriched := Enrich(`time="2024-10-09T08:03:05Z" level=debug msg="Skipping endpoint test-userid-service-tc2.api.qa.example.dev 300 IN A  203.0.113.15 [] because owner id does not match, found: \"acme-qa-we\", required: \"dev-we\""`)
	assert.Equal(t, "debug", enriched.Severity)
	assert.Equal(t, "2024-10-09 08:03:05 +0000 UTC", enriched.Time.String())
}

func TestEnrich_DevDocs(t *testing.T) {
	enriched := Enrich(`[09/10/2024 08:19:42 INFO devdocshub.storage 9] Refresh loop finished devdocs`)
	assert.Equal(t, "info", enriched.Severity)
	assert.Equal(t, "2024-10-09 08:19:42 +0000 UTC", enriched.Time.String())
}

func TestEnrich_LowCoder(t *testing.T) {
	enriched := Enrich(`2024-10-09 08:18:10.250 INFO org.lowcoder.runner.task.IoHeartBeatTask#lambda$ping$0:34    [lettuce-epollEventLoop-9-1]: schedule ping executed, result: true`)
	assert.Equal(t, "info", enriched.Severity)
	assert.Equal(t, "2024-10-09 08:18:10.25 +0000 UTC", enriched.Time.String())
}

func TestEnrich_WatchLoop(t *testing.T) {
	enriched := Enrich(`2024-10-09 08:43:27 starting watch loop"`)
	assert.Empty(t, enriched.Severity)
	assert.Equal(t, "2024-10-09 08:43:27 +0000 UTC", enriched.Time.String())
}

func TestEnrich_OpenPolicyAgent(t *testing.T) {
	enriched := Enrich(`2024/10/08 06:54:03 http: TLS handshake error from 172.19.10.25:47598: EOF`)
	assert.Empty(t, enriched.Severity)
	assert.Equal(t, "2024-10-08 06:54:03 +0000 UTC", enriched.Time.String())
}

func TestEnrich_AzureCNS(t *testing.T) {
	enriched := Enrich(`2024/10/08 06:56:51 [1] [releaseIPConfigs] Releasing pod with key aab53256-eth0"`)
	assert.Empty(t, enriched.Severity)
	assert.Equal(t, "2024-10-08 06:56:51 +0000 UTC", enriched.Time.String())
}

func TestEnrich_Configmap_Reload(t *testing.T) {
	enriched := Enrich(`2024/10/08 06:36:55 Watching directory: "/etc/alloy"`)
	assert.Empty(t, enriched.Severity)
	assert.Equal(t, "2024-10-08 06:36:55 +0000 UTC", enriched.Time.String())
}

func TestEnrich_Azdo_Agent(t *testing.T) {
	enriched := Enrich(`2024-10-08 06:32:14Z: Job Deploy acme-qa completed with result: Succeeded`)
	assert.Empty(t, enriched.Severity)
	assert.Equal(t, "2024-10-08 06:32:14 +0000 UTC", enriched.Time.String())
}

func TestEnrich_Github_Agent(t *testing.T) {
	enriched := Enrich(`2024-10-08 06:34:30Z: Running job: Test packages`)
	assert.Empty(t, enriched.Severity)
	assert.Equal(t, "2024-10-08 06:34:30 +0000 UTC", enriched.Time.String())
}

func TestEnrich_Oauth2Proxy(t *testing.T) {
	enriched := Enrich(`244.2.33:35634 - d131ad61e0daeb93e02b0f6b9f6a8260 - foo@bar.com [2024/10/08 06:46:40] oauth2-proxy.ingress.svc.cluster.local GET - "/oauth2/auth" HTTP/1.1 "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/129.0.0.0 Safari/537.36" 202 0 0.000`)
	assert.Equal(t, "info", enriched.Severity)
	assert.Equal(t, "2024-10-08 06:46:40 +0000 UTC", enriched.Time.String())
}

func TestEnrich_Oauth2Proxy_Fail(t *testing.T) {
	enriched := Enrich(`172.19.5.6:48444 - 9d3111704c7e2e31616778934f522c10 - - [2024/10/08 06:47:55] oauth2-proxy.ingress.svc.cluster.local GET - "/oauth2/auth" HTTP/1.1 "Azure Traffic Manager Endpoint Monitor" 401 13 0.00`)
	assert.Equal(t, "warn", enriched.Severity)
	assert.Equal(t, "2024-10-08 06:47:55 +0000 UTC", enriched.Time.String())
}

func TestEnrich_HttpEcho(t *testing.T) {
	enriched := Enrich(`2024/10/08 06:38:47 blackbox-ping-we.api.afr.example.net 172.19.0.7:54380 "GET / HTTP/1.1" 200 12 "Blackbox Exporter/" 6.68µs"`)
	assert.Equal(t, "info", enriched.Severity)
	assert.Equal(t, "2024-10-08 06:38:47 +0000 UTC", enriched.Time.String())
}

func TestEnrich_GoMicro(t *testing.T) {
	enriched := Enrich(`2024/11/05 14:13:11.400089 Cost for acme-infrastructure: 677.652104`)
	assert.Equal(t, "2024-11-05 14:13:11.400089 +0000 UTC", enriched.Time.String())
}

func TestEnrich_Envoy_ERROR(t *testing.T) {
	enriched := Enrich(`{"upstream_local_address":"172.19.5.94:33042","duration":2001,"path":"/healthz/running","method":"GET","bytes_received":0,"protocol":"HTTP/1.1","downstream_remote_address":"172.19.10.17:45998","downstream_local_address":"172.19.5.94:8443","response_flags":"ERROR","response_code":0,"requested_server_name":"feed-master.api.example.dev","x_forwarded_for":"172.19.10.17","request_id":"00000000-0000-0000-0000-000000000019","upstream_cluster":"feed-proxy-master_base-feed-proxy_80","@timestamp":"2024-11-25T14:57:23.461Z","bytes_sent":0,"upstream_host":"172.19.3.36:5000","authority":"feed-master.api.example.dev"}`)
	assert.Equal(t, "error", enriched.Severity)
}

func TestEnrich_Envoy_HTTP_Downstream_Reset(t *testing.T) {
	enriched := Enrich(`{"x_forwarded_for":"203.0.113.13,203.0.113.13,203.0.113.19","response_code":0,"@timestamp":"2025-03-11T11:03:05.461Z","method":"POST","user_agent":"Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/133.0.0.0 Mobile Safari/537.36","bytes_sent":0,"requested_server_name":"graphql-live-we.api.afr.example.net","upstream_host":"172.19.1.58:5001","downstream_local_address":"172.19.17.17:8443","request_id":"00000000-0000-0000-0000-000000000020","path":"/graphql?nogeoredirect=1","duration":10,"bytes_received":1962,"protocol":"HTTP/2","upstream_local_address":"172.19.17.17:53620","response_flags":"DR","upstream_cluster":"base-infra-graphql-live_gateway_5001","authority":"graphql-live-we.api.afr.example.net","downstream_remote_address":"203.0.113.19:61866"}`)
	assert.Equal(t, "2025-03-11 11:03:05.461 +0000 UTC", enriched.Time.String())
	assert.Equal(t, "warn", enriched.Severity)
}

func TestEnrich_Envoy_HTTP_Downstream_Disconnect(t *testing.T) {
	enriched := Enrich(`{"method":"POST","x_forwarded_for":"172.19.18.38","downstream_local_address":"172.19.9.125:8443","downstream_remote_address":"172.19.18.38:40336","bytes_sent":0,"@timestamp":"2024-11-25T15:02:08.288Z","path":"/offer.v1.OfferService/SubscribeChangedEvents","bytes_received":32,"request_id":"0f6d15d387356faad94cd12dca767dd3","upstream_local_address":"172.19.9.125:34028","response_code":0,"upstream_host":"172.19.18.120:5000","upstream_cluster":"offer-service-master_basebook-core-mapper_5000","response_flags":"DC","requested_server_name":"offer-master.api.example.dev","user_agent":"grpc-dotnet/2.65.0 (.NET 8.0.11; CLR 8.0.11; net8.0; linux; arm64) Acme-Offer-Domain/20241122.1","protocol":"HTTP/2","authority":"offer-master.api.example.dev","duration":12009}`)
	assert.Equal(t, "warn", enriched.Severity)
}

func TestEnrich_MemcachedExporter(t *testing.T) {
	enriched := Enrich(`time=2025-01-09T10:53:50.011Z level=INFO source=tls_config.go:350 msg="TLS is disabled." http2=false address=[::]:9150`)
	assert.Equal(t, "2025-01-09 10:53:50.011 +0000 UTC", enriched.Time.String())
	assert.Equal(t, "info", enriched.Severity)
}

func TestEnrich_ACR(t *testing.T) {
	enriched := Enrich(`{ "time": "2025-01-30T14:19:15.1931730Z", "category": "ContainerRegistryRepositoryEvents", "resourceId": "/SUBSCRIPTIONS/00000000-0000-0000-0000-000000000009/RESOURCEGROUPS/DOCKER/PROVIDERS/MICROSOFT.CONTAINERREGISTRY/REGISTRIES/EXAMPLE", "operationName": "Pull", "identity": "user@example.com", "location": "westeurope", "resultType": "HttpStatusCode", "resultDescription": "404", "durationMs": "38.212784", "callerIpAddress": "203.0.113.18", "correlationId": "00000000-0000-0000-0000-000000000021", "properties": {"tag":"sha256:2a871364028e4fcc58cb139239390bc242fa7acee3aaa22ab200422d7682084c","digest":"<nil>","mediaType":"<nil>","repository":"tipster-service","artifactType":"acr.docker","tenantId":"00000000-0000-0000-0000-000000000011","loginServer":"example.azurecr.io","userAgent":"acr-cleanup azsdk-go-azcontainerregistry/v0.2.2 (go1.23.5; linux)"}, "RegionStamp": "weu-3"}`)
	assert.Equal(t, "2025-01-30 14:19:15.193173 +0000 UTC", enriched.Time.String())
	assert.Equal(t, "warn", enriched.Severity)
}

func TestEnrich_Envoy_TCP(t *testing.T) {
	enriched := Enrich(`{"upstream_host":"172.19.6.110:5672","@timestamp":"2025-03-11T10:24:28.592Z","response_flags":"-","duration":252,"upstream_cluster":"base-baseintegration-xapiproxy_rabbitmq-internal_5672","bytes_received":184,"bytes_sent":8,"downstream_local_address":"172.19.6.88:8443","downstream_remote_address":"203.0.113.8:38663","upstream_local_address":"172.19.6.88:57520","response_code":0,"requested_server_name":"xapi-mq-internal-we.row-uat.example.net"}`)
...
func TestKafka(t *testing.T) {
	enriched := Enrich(`%4|1744173504.870|FAIL|rdkafka#producer-1| [thrd:sasl_plaintext://kafka-cluster-kafka-bootstrap.kafka.svc.cluste]: sasl_plaintext://kafka-cluster-kafka-bootstrap.kafka.svc.cluster.local:9098/bootstrap: Connection setup timed out in state CONNECT (after 30029ms in state CONNECT)`)
	assert.Equal(t, "2025-04-09 04:38:24.869999885 +0000 UTC", enriched.Time.String())
	assert.Equal(t, "warn", enriched.Severity)
}

func TestEnrich_Mirrormaker(t *testing.T) {
	enriched := Enrich(`2025-05-06 06:08:24,950 INFO 172.25.1.8 - - [06/May/2025:06:08:24 +0000] "GET / HTTP/1.1" 200 91 "-" "kube-probe/1.31" 1 (org.apache.kafka.connect.runtime.rest.RestServer) [qtp1239462179-7686]`)
	assert.Equal(t, "2025-05-06 06:08:24.95 +0000 UTC", enriched.Time.String())
	assert.Equal(t, "info", enriched.Severity)
}

func TestEnrich_Mirrormaker_NoMillis(t *testing.T) {
	enriched := Enrich(`2025-05-06 06:10:14 INFO  AbstractOperator:546 - Reconciliation #1916(timer) KafkaMirrorMaker2(offer-popularity-service/offer-popularity): reconciled`)
...
func TestNormalizeSeverity(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.input, func(t *testing.T) {
			level, number := NormalizeSeverity(c.input)
...
func TestEnrich_Configuration_Tag_Not_Fatal(t *testing.T) {
	enriched := Enrich(`2026/06/16 14:20:46 [configuration] invalid IPv6PrefixClamp value 0; must be between 120 to 128, defaulting to /120`)
...
func TestEnrich_LokiClientDoFailure_StaysError(t *testing.T) {
	enriched := Enrich(`level=error ts=2026-06-29T13:42:09.790718438Z caller=client.go:522 index-store=tsdb-2020-01-01 msg="client do failed for instance 172.19.13.12:9095" err="rpc error: code = Unavailable desc = connection refused"`)
	assert.Equal(t, "error", enriched.Severity)
}
